...
use Fab\VidiLight\Persistence\Storage\VidiDbBackend;
...
class Query implements QueryInterface
{
    ...
    public function getTypo3QuerySettings()
    {
        ...
        if ($this->isBackendMode()) {
            ...
            foreach ($querySettings as $setting) {
                if (
                    isset(
                        $configuration['config.']['tx_vidi.']['persistence.']['backend.'][$this->type . '.'][$setting],
                    )
                ) {
                    $value =
                        (bool) $configuration['config.']['tx_vidi.']['persistence.']['backend.'][$this->type . '.'][
                            $setting
                        ];
    ...
    public function logicalAnd($constraint1)
    {
        ...
        if ($resultingConstraint === null) {
            throw new InvalidNumberOfConstraintsException(
                'There must be at least one constraint or a non-empty array of constraints given.',
                1401289500,
            );
        }
    ...
    public function in($propertyName, $operand)
    {
        if (!is_array($operand) && !$operand instanceof \ArrayAccess && !$operand instanceof \Traversable) {
            throw new UnexpectedTypeException(
                'The "in" operator must be given a mutlivalued operand (array, ArrayAccess, Traversable).',
                1264678095,
            );
        }
        return $this->qomFactory->comparison(
