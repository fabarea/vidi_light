...
namespace Fab\VidiLight\Persistence\Storage;
...
use Fab\VidiLight\Resolver\FieldPathResolver;
use Doctrine\DBAL\ParameterType;
use Fab\VidiLight\Persistence\Query;
use Fab\VidiLight\Utility\BackendUtility;
...
class VidiDbBackend
{
    ...
    public function countResult()
    {
        ...
        if (!empty($statementParts['limit'])) {
            ...
            $count = $this->getConnection()->executeQuery($sql, $parameters, $types)->rowCount();
        } else {
            $statementParts['fields'] = ['COUNT(*)'];
            ...
            if (isset($statementParts['keywords']['distinct'])) {
                ...
                $statementParts['fields'] = [
                    'COUNT(DISTINCT ' . $statementParts['mainTable'] . '.' . $distinctField . ')',
                ];
            }

            $sql = $this->buildQuery($statementParts);
            $count = $this->getConnection()->executeQuery($sql, $parameters, $types)->fetchColumn(0);
        }
        return (int) $count;
    }
    ...
    public function buildQuery(array $statementParts)
    {
        ...
        if (!empty($statementParts['unions'])) {
            foreach ($statementParts['unions'] as $tableName => $unionPart) {
                if (!empty($statementParts['additionalWhereClause'][$tableName])) {
                    $statementParts['unions'][$tableName] .=
                        ' AND ' . implode(' AND ', $statementParts['additionalWhereClause'][$tableName]);
                }
            }
        }
        ...
        $statement =
            'SELECT ' .
            implode(' ', $statementParts['keywords']) .
            ' ' .
            implode(',', $statementParts['fields']) .
            ' FROM ' .
            implode(' ', $statementParts['tables']) .
            ' ' .
            implode(' ', $statementParts['unions']);
        if (!empty($statementParts['where'])) {
            ...
            if (!empty($statementParts['additionalWhereClause'][$this->query->getType()])) {
                $statement .=
                    ' AND ' . implode(' AND ', $statementParts['additionalWhereClause'][$this->query->getType()]);
            }
        } elseif (!empty($statementParts['additionalWhereClause'])) {
            $statement .=
                ' WHERE ' . implode(' AND ', $statementParts['additionalWhereClause'][$this->query->getType()]);
        }
        ...
        if ($input instanceof \DateTime) {
            ...
            if ($realInput instanceof DomainObjectInterface) {
                ...
                throw new UnexpectedTypeException(
                    'An object of class "' . get_class($realInput) . '" could not be converted to a plain value.',
                    1274799934,
                );
            }
    ...
    protected function parseDynamicOperand(
        ...
        if ($operand instanceof LowerCaseInterface) {
            ...
            if (isset($tableName) && !empty($this->currentChildTableNameAlias)) {
                $constraintSQL = $this->replaceTableNameByAlias(
                    $tableName,
                    $this->currentChildTableNameAlias,
                    $constraintSQL,
                );
    ...
    protected function addUnionStatement(&$tableName, &$propertyPath, array &$statementParts)
    {
        ...
        if ($childTableName === null) {
            throw new InvalidRelationConfigurationException(
                'The relation information for property "' . $fieldName . '" of class "' . $tableName . '" is missing.',
                1353170925,
            );
        }
        ...
        if ($table->field($fieldName)->hasOne()) {
            ...
            if ($table->field($fieldName)->hasRelationManyToOne() || $parentKeyFieldName === null) {
                $statementParts['unions'][$childTableName] =
                    'LEFT JOIN ' .
                    $childTableName .
                    ' ON ' .
                    $tableName .
            ...
            if ($tableNameCondition) {
                ...
                if (empty($sourceFileName)) {
                    ...
                    $additionalMMConditions = Tca::table($tableNameCondition)
                        ->field($sourceFileName)
                        ->getAdditionalMMCondition();
                }

                foreach ($additionalMMConditions as $additionalFieldName => $additionalMMCondition) {
                    $additionalJoin = sprintf(
                        ' AND %s.%s = "%s"',
                        $relationTableNameAlias,
                        $additionalFieldName,
                        $additionalMMCondition,
                    );
     ...
     * @return void
     */
    protected function addAdditionalWhereClause(
     ...
     * @return void
     */
    protected function addVisibilityConstraintStatement(
        QuerySettingsInterface $querySettings,
        $tableNameOrAlias,
        array &$statementParts,
    ) {
        ...
        if (is_array($GLOBALS['TCA'][$tableName]['ctrl'])) {
            ...
            if (Typo3Mode::isFrontendMode()) {
                ...
                // 'BE' case
                $statement .= $this->getBackendConstraintStatement(
                    $tableNameOrAlias,
                    $ignoreEnableFields,
                    $includeDeleted,
                );
     ...
     */
    protected function getFrontendConstraintStatement(
        $tableNameOrAlias,
        $ignoreEnableFields,
        $enableFieldsToBeIgnored,
        $includeDeleted,
        ...
        if ($ignoreEnableFields && !$includeDeleted) {
            if (count($enableFieldsToBeIgnored)) {
                ...
                $statement .= $this->getPageRepository()->enableFields(
                    $tableName,
                    -1,
                    array_combine($enableFieldsToBeIgnored, $enableFieldsToBeIgnored),
                );
            } else {
            ...
            throw new InconsistentQuerySettingsException(
        ...
        if (is_array($GLOBALS['TCA'][$tableName]['ctrl'])) {
            if (!empty($GLOBALS['TCA'][$tableName]['ctrl']['languageField'])) {
                ...
                $additionalWhereClause =
                    ...
                    ' IN (' .
                    intval($querySettings->getLanguageUid()) .
                    ',-1)';
                ...
                if (
                    ...
                    if (isset($GLOBALS['TCA'][$tableName]['ctrl']['delete'])) {
                        $additionalWhereClause .=
                            ' AND ' . $tableNameOrAlias . '.' . $GLOBALS['TCA'][$tableName]['ctrl']['delete'] . '=0';
                    }
                    $additionalWhereClause .= '))';
                }
    ...
    protected function getContentObjects(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $overlaidRow = $this->doLanguageAndWorkspaceOverlay($row, $this->query->getTypo3QuerySettings());

            $contentObjects[] = GeneralUtility::makeInstance(Content::class, $this->query->getType(), $overlaidRow);
        }
    ...
    protected function doLanguageAndWorkspaceOverlay(array $row, $querySettings)
    {
        ...
        if (isset($GLOBALS['TSFE']) && is_object($GLOBALS['TSFE'])) {
            $languageMode = GeneralUtility::makeInstance(Context::class)->getPropertyFromAspect(
                'language',
                'legacyLanguageMode',
            );
