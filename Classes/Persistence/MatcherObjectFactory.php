...
use Fab\VidiLight\Module\ModuleName;
...
class MatcherObjectFactory implements SingletonInterface
{
    ...
    {
        $tsConfig = $this->getBackendUser()->getTSConfig()['tx_vidi.']['dataType.'][$matcher->getDataType() . '.'][
            'constraints.'
        ];
    ...
    protected function parseQuery(array $queryParts, Matcher $matcher): Matcher
    {
        ...
        foreach ($queryParts as $term) {
            ...
            if (
                Tca::grid($resolvedDataType)->hasFacet($fieldName) &&
                Tca::grid($resolvedDataType)->facet($fieldName)->canModifyMatcher()
            ) {
    ...
    {
        return (Tca::table($dataType)->field($fieldName)->hasRelation() &&
    ...
    protected function emitPostProcessMatcherObjectSignal(Matcher $matcher): void
    {
        ...
        $this->getSignalSlotDispatcher()->dispatch(
            'Fab\VidiLight\Controller\Backend\ContentController',
            'postProcessMatcherObject',
            [$matcher, $matcher->getDataType()],
        );
    }
